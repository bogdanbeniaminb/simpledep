...
it('handles install requests', function () {
  ...
  expect($request)->toBeInstanceOf(Request::class)
    ->and($request->getType())->toBe(Request::TYPE_INSTALL)
    ->and($request->getName())->toBe('foo')
    ->and($request->getVersionConstraint())->toBeNull();

  $request = new Request(Request::TYPE_INSTALL, 'foo', '>=1.0.0');
  expect($request->getVersionConstraint())->toBeInstanceOf(Constraint::class)
    ->and((string) $request->getVersionConstraint())->toBe('>=1.0.0');
...
it('handles uninstall requests', function () {
  ...
  expect($request->getType())->toBe(Request::TYPE_UNINSTALL)
    ->and($request->getName())->toBe('foo')
    ->and($request->getVersionConstraint())->toBeNull();
});
...
it('handles update requests', function () {
  ...
  expect($request->getType())->toBe(Request::TYPE_UPDATE)
    ->and($request->getName())->toBe('foo')
    ->and($request->getVersionConstraint())->toBeNull();
